...
export default function AiAssistant() {
  ...
  useEffect(() => {
    ...
    if (uid && uid !== "guest_user") {
        ...
        fetch(`http://localhost:8000/api/ai/history/${uid}`)
            .then(res => {
            ...
            return (
                <div key={idx} className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`flex flex-col max-w-[85%] sm:max-w-[75%] ${isUser ? "items-end" : "items-start"}`}>
                        ...
                        {m.file && (
                            <div className={`mb-1 px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-2 border ${
                                isUser ? "bg-blue-700 text-blue-100 border-blue-600" : "bg-white text-gray-600 border-gray-200"
                            }`}>
                                <FileText size={14} />
                                {m.file.replace("📎 ", "")}
                            </div>
